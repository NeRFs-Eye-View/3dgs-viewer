...
async function fetchInChunks(url, onChunkReceived) {
...
function processChunk(chunk, chunks, worker, isPly, textureReady, rowLength, bytesRemains) {
    chunks.push(chunk);
    if (!textureReady) return;

    const bytesDownloaded = chunks.reduce((sum, chunk) => sum + chunk.length, 0);
    if (bytesDownloaded - bytesRemains > rowLength) {
        let vertexCount = Math.floor((bytesDownloaded - bytesRemains) / rowLength);
        const concatenatedChunksBuffer = new Uint8Array(bytesDownloaded);
        let offset = 0;
        ...
        chunks.length = 0;
        if (bytesDownloaded - vertexCount * rowLength > 0) {
            const extraData = new Uint8Array(bytesDownloaded - vertexCount * rowLength);
            extraData.set(concatenatedChunksBuffer.subarray(bytesDownloaded - extraData.length), 0);
            chunks.push(extraData);
        }
        const buffer = new Uint8Array(vertexCount * rowLength);
...
async function main() {
    ...
    fetchInChunks(url, (chunk) => {
        ...
        if (bytesDownloaded - 0 > 0) {
            ...
            if (isPly) {
                concatenatedChunks = new Uint8Array(processPlyBuffer(concatenatedChunks.buffer));
            }
            worker.postMessage({
                method: 'push',
                buffer: concatenatedChunks.buffer,
                vertexCount: Math.floor(concatenatedChunks.byteLength / rowLength)
...
function createWorker(self) {
    ...
    self.onmessage = (e) => {
        if (e.data.method === 'clear') {
            ...
            vertexCount += e.data.vertexCount;
            pushDataBuffer(e.data.buffer, e.data.vertexCount);
        } else if (e.data.method === 'sort') {
            sortBuffer(e.data.view);
        }
    };
    ...
    function sortBuffer(view) {
        const viewProj = new Float32Array(view);
        const f_buffer = new Float32Array(buffer.flat());

        let vertexDepths = [];
        for (let i = 0; i < vertexCount; i++) {
            const x = f_buffer[i * 8];
            const y = f_buffer[i * 8 + 1];
            const z = f_buffer[i * 8 + 2];

            const depth = viewProj[2] * x + viewProj[6] * y + viewProj[10] * z + viewProj[14];
            vertexDepths.push({ index: i, depth: depth });
        }

        vertexDepths.sort((a, b) => b.depth - a.depth);
        const sortedIndices = vertexDepths.map(d => d.index);
