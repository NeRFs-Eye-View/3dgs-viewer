...
void main () {
    ...
    float det_cov = cov2d[0][0] * cov2d[1][1] - cov2d[0][1] * cov2d[0][1];
    cov2d[0][0] += h_var;
    cov2d[1][1] += h_var;
    float det_cov_plus_h_var = cov2d[0][0] * cov2d[1][1] - cov2d[0][1] * cov2d[0][1];

    float h_convolution_scaling = sqrt(max(0.000025, det_cov / det_cov_plus_h_var)); // Ensure numerical stability
    ...
    float depthFactor = clamp(pos2d.z / pos2d.w + 1.0, 0.0, 1.0);

    float r = float((cov.w)        & 0xffu) / 255.0;
    float g = float((cov.w >> 8)  & 0xffu) / 255.0;
    float b = float((cov.w >> 16) & 0xffu) / 255.0;
    float a = float((cov.w >> 24) & 0xffu) / 255.0;

    vec3 baseRGB  = depthFactor * vec3(r, g, b);
    float baseAlpha = depthFactor * a;

    float finalAlpha = baseAlpha * h_convolution_scaling;

    vColor = vec4(baseRGB, finalAlpha);
